<script>
    ...
    import InputYear from '../input/InputYear.svelte';
    ...
    function adjust( sires, dames ) {

        const newParents = [];

        for( let parent of parents ) {
            if( sires > 0 && parent.sex == '1.0' ) {
                newParents.push( parent );
                sires--;
            }
        }
        while( sires > 0 ) {
            newParents.push( {sex:'1.0', ring:{ country:'D', year:new Date().getFullYear()-1, code:null }, score: null, parents: { id:null, breeder:null, year:2021, name:null } } );
            sires--;
        }

        for( let parent of parents ) {
            if( dames > 0 && parent.sex == '0.1' ) {
                newParents.push( parent );
                dames--;
            }
        }
        while( dames > 0 ) {
            newParents.push( {sex:'0.1', ring:{ country:'D', year:new Date().getFullYear()-1, code:null }, score: null, parents: { id:null, breeder:null, year:2021, name:null } } );
            dames--;
        }
        parents = newParents;
    }
...
<div class='w-full flex flex-col p-2 gap-1'>
    ...
    <div class='flex flex-row align-bottom'>
        <InputNumber class='w-12' min=0 max=99 bind:value={sires} label='Hähne' {disabled}/>
        <span>.</span>
        <InputNumber class='w-12' min=0 max=99 bind:value={dames} label='Hennen' {disabled}/>
    </div>
