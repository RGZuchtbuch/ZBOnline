...
<!-- mobile -->
<div class='md:hidden flex flex-col justify-center align-middle gap-2 bg-header px-2 py-1 text-white no-print'>
    <a class='p-2 text-center' class:active={ $route.url.startsWith('/zuchtbuch') } href='/zuchtbuch'>Das Zuchtbuch</a>
    <a class='p-2 text-center' class:active={ $route.url.startsWith('/verband') } href='/verband'>Verbände</a>
    {#if STANDARDENABLED}<a class='p-2 text-center' class:active={ $route.url.startsWith('/standard') } href='/standard'>Standard</a>{/if}

    {#if RESULTSENABLED}<a class='p-2 text-center' class:active={ $route.url.startsWith('/leistungen') } href='/leistungen'>Leistungen</a>{/if}
    {#if BREEDERENABLED && $user } <a class='p-2 text-center' class:active={ $route.url.startsWith('/zuechter') } href='/zuechter'>Mein Zuchtbuch</a> {/if}
    {#if MODERATORENABLED && $user && $user.moderator.length > 0 } <a class='p-2 text-center' class:active={ $route.url.startsWith('/obmann') } href='/obmann'>Obmann</a> {/if}
    {#if ADMINENABLED && $user && $user.admin } <a class='p-2 text-center' class:active={ $route.url.startsWith('/admin') } href='/admin'>Admin</a> {/if}
    <!-- no login -->
</div>

<!-- wide screen -->
<div class='hidden w-full md:flex flex-row gap-2 bg-header px-2 py-1 text-white no-print'>
    <div class='w-48'></div>
