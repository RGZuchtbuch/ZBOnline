<script>
	...
	let { section, breed=$bindable(), unfold=false } = $props();
			...
			<Form autosubmit onsubmit={onSubmit}>
				<fieldset class='ml-8 flex flex-col px-2'>
					<div class='flex flex-row gap-x-2'>
						...
						<div class='grow'></div>
						<CheckBox bind:value={ remove } label='Löschen' title='Nur wenn Name leer, keine Farben und keine Meldungen' disabled={breed.name || breed.colors.length > 0 } />
						<Status />
					</div>
			...
			<div class='pl-12' transition:slide={{duration:breed.colors.length*25}}>
				{#if breed.colors.length > 0}
					{#each breed.colors as color, i}
						{#key color}
							<Color {breed} bind:color={breed.colors[i]}/><!-- compilers said using index is better for binding -->
						{/key}
					{/each}
				{:else}
					<div>Noch keine farbenschläge eingegeben</div>
				{/if}
			</div>
		{/if}
	{/if}
