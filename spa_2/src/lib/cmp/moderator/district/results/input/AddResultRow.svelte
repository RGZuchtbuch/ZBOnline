<script>
    ...
    function calcPeriodDays( first, last ) {
        ...
        addedResult.layPeriod.days = daysBetween( first, last );
    }

    function calcProduction( event ) {
        console.log( 'calcProduction' );
        addedResult.layPeriod.production = Math.round( getLayResult( addedResult.layPeriod.days, addedResult.layPeriod.eggs, addedResult.layPeriod.dames ) );
    }
