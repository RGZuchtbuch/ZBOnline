<script>
	...
	function setHeader() {
		...
		ctx.submenu = [
			{name: 'Stämme', href: `/admin/district/${ctx.district.id}/breeder/${ctx.breeder.id}/pair`},
			{name: 'Mitglied', href: `/admin/district/${ctx.district.id}/breeder/${ctx.breeder.id}/profile`},
		];
		ctx.crumbs = [
			//{name: 'Start', href: '/'},
